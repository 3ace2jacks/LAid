...
from course.views import UserCourseList, CourseDetail, AvailableCourseList, JoinCourse, \
    CourseLectureList, LectureDetail
...
urlpatterns = [
    ...
    # Courses urls
    url(r'^courses/(?P<pk>[0-9]+)/lectures/$', CourseLectureList.as_view(),
        name="course_lecture_list"),
    ...
    # Lectures urls
    url(r'^lectures/(?P<pk>[0-9]+)/questions/$', QuestionList.as_view(), name='question_list'),
    url(r'^lectures/(?P<pk>[0-9]+)/flow/count/(?P<tk>[0-9]+)$', FlowCount.as_view(),
        name='flow_count'),
    url(r'^lectures/(?P<pk>[0-9]+)/flow/count/$', FlowCount.as_view(),
        name='flow_count_default'),
    url(r'^lectures/(?P<pk>[0-9]+)/flow/$', FlowList.as_view(), name='flow_list'),
    url(r'^lectures/(?P<pk>[0-9]+)/$', LectureDetail.as_view(), name="lecture_detail"),

    # Question urls
    url(r'^questions/(?P<pk>[0-9]+)/answer/$', AnswerLiveQuestion.as_view(),
        name='live_question_answer'),
